...
separations of $5$, $10$, and $20\,\hinvMpc$, % CHECK: 20 hMpc count
...
three separations, at $0.7\sigma$, $2.2\sigma$, and $-1.1\sigma$. % CHECK
Testing the compressed profile against zero with the full covariance gives
the same conclusion. The mock predicts a bridge of roughly $6$, $3.4$, and
$1.5\times10^{-4}$ in the same statistic, % CHECK
below our errors at every separation, so the observations are consistent
with the prediction and equally consistent with no bridge at all. The
$95$ per cent upper limits correspond to % TODO: Sigma and linear mass density limits
The $2.2\sigma$ excess at $10\,\hinvMpc$ is at the level of the
...
doing: the predicted bridge falls by a factor of four from $5$ to
...
our three bins with inverse-variance weights gives % TODO
which is the number to compare with theirs.

\textbf{The limit is the map, not the method.} Removing the control
entirely and measuring the same window on the raw band changes the error by
% TODO: quote from the committed pipeline (the 7% figure came from the fitted-control analysis)
