...
$5\,\hinvMpc$, but by only a few percent at $10$ and $20\,\hinvMpc$ --- so
...
from the mock, so such preference as there is runs mildly toward the
...
with theirs. The three samples are drawn from one catalog over one
...
given $X$ lies closer to the halo than the off-center band, an isotropic
...
should be read as bridge plus halo leakage. % TODO: quote the mock leakage estimate (B(X) with the filament removed, or with randomized pair orientations)
...
catalogs over smaller areas, and a joint galaxy-shape and CMB-lensing
