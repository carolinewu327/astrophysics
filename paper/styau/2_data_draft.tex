...
of the CMASS sample and its convergence field provides a $\Lambda$CDM
reference signal
(Section~\ref{sec:sim}).
...
$\simeq3.3\,\hinvMpc$ comoving. Section~\ref{sec:robust} compares this
fiducial choice with the available $4$- and $6$-arcmin results. We sample the
smoothed map at the sky positions
...
We calculate the $\Lambda$CDM reference signal using BigMDPL from the
MultiDark suite \citep{Klypin2016, Riebe2013}. This dark-matter-only simulation
evolves $3840^{3}$ particles in a periodic cube of comoving side length
$2500\,\hinvMpc$; each particle has mass
$2.36\times10^{10}\,h^{-1}\mathrm{M}_{\odot}$. Its cosmological parameters
include $\Omega_{\rm m}=0.307$ and $h=0.678$. We analyze the output at
$z=0.547$ ($a=0.6464$), which is close to the weighted mean redshift of the
CMASS sample. Haloes
...
$\Sigma_{\rm crit}=8.88\times10^{14}\,h\,\mathrm{M}_{\odot}\,\mathrm{Mpc}^{-2}$.
...
and we map each galaxy to redshift space by shifting its coordinate along
this axis by $(1+z)\,v_{\parallel}/H(z)$, with $v_{\parallel}$ denoting the
peculiar-velocity component along the same axis. We evaluate the
line-of-sight separation cut from these shifted coordinates. Transverse
